...
# Packages charts/kubestash-certified, drops it into a local checkout of
# appscode/openshift-helm-charts, and runs redhat-certification/chart-verifier
# against it to produce a certification report.
#
# Usage: openshift-chart-verify.sh <kubestash-license-path> [version]
#   kubestash-license-path  path to a KubeStash Enterprise license file;
...
#                           charts/kubestash-certified/Chart.yaml).
#
...
if [ $# -lt 1 ]; then
    echo "Usage: $0 <kubestash-license-path> [version]" >&2
    exit 1
fi
...
CHART_VERSION="${2:-$(grep -E '^version:' "$REPO_ROOT/charts/kubestash-certified/Chart.yaml" | awk '{print $2}')}"
...
helm package "$REPO_ROOT/charts/kubestash-certified" --destination "$PACKAGE_DIR"

CHART_DEST_DIR="$OPENSHIFT_HELM_CHARTS_DIR/charts/partners/appscode/kubestash-certified/$CHART_VERSION"
mkdir -p "$CHART_DEST_DIR"

CHART_PACKAGE="$(find "$PACKAGE_DIR" -maxdepth 1 -name 'kubestash-certified-*.tgz')"
CHART_TGZ="$CHART_DEST_DIR/kubestash-certified-$CHART_VERSION.tgz"
cp "$CHART_PACKAGE" "$CHART_TGZ"

echo "==> Installing kubestash-certified-crds CRDs into the cluster"
helm upgrade --install kubestash-certified-crds "$REPO_ROOT/charts/kubestash-certified-crds"
