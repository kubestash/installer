...
SRC_PKGS := apis tests hack/fmt # directories which hold app source (not vendored)
SRC_DIRS := $(SRC_PKGS)
...
gen-values-schema: $(BUILD_DIRS)
	@for dir in charts/*/; do \
		dir=$${dir%*/}; \
		dir=$${dir##*/}; \
		crd_file=.crds/installer.kubestash.com_$$(echo $$dir | tr -d '-')s.yaml; \
		if [ ! -f $${crd_file} ]; then \
			continue; \
		fi; \
...
update-certified-charts: update-chart-dependencies install-chart-packer
	...
	PATH="$(BIN_DIR):$$PATH" chart-packer crd-less --input charts/kubestash --output charts
	PATH="$(BIN_DIR):$$PATH" chart-packer crd-only --input charts/kubestash --output charts
	...
	@if ! git diff --quiet -- charts/kubestash-certified/Chart.lock; then \
		changed=$$(git diff -U0 -- charts/kubestash-certified/Chart.lock | grep -E '^[+-]' | grep -vE '^(\+\+\+|---)' | grep -vE '^[+-]generated:'); \
		if [ -z "$$changed" ]; then \
			echo "Reverting charts/kubestash-certified/Chart.lock (only generated: timestamp changed)"; \
			git checkout -- charts/kubestash-certified/Chart.lock; \
		fi; \
	fi
